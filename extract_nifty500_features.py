...
# Path of the Nifty 500 folder
data_dir = os.path.join(workspace, "data_nifty500_10y")
nifty_path = os.path.join(data_dir, "NIFTY_50_day.csv")
...
def calculate_indicators(df):
    ...
    df['bbw'] = (3.0 * df['std20'] / df['sma20']) * 100
    ...
    df = df.drop(columns=['s', 's_zero', 'last_zero_s', 'tr'])
    return df
...
def extract_features_at_signal(df, idx):
    ...
    volume_multiple = row['volume'] / vol_avg if vol_avg > 0 else 1.0
    ...
    atr_pct = (row['atr14'] / row['close']) * 100
    
    sma50 = row['sma50']
    distance_from_50sma = (row['close'] - sma50) / sma50 if sma50 > 0 else 0.0
...
def main():
    ...
    print("Starting Nifty 500 ML feature extraction...")
    
    # Load Nifty 50 daily index to map trends
    nifty_lookup = {}
    nifty_dates = []
    if os.path.exists(nifty_path):
        try:
            nifty_df = pd.read_csv(nifty_path)
            nifty_df['date_parsed'] = pd.to_datetime(nifty_df['date'].astype(str).str.slice(0, 10))
            nifty_df = nifty_df.sort_values('date_parsed').reset_index(drop=True)
            nifty_df['sma50'] = nifty_df['close'].rolling(window=50).mean()
            
            date_to_nifty_idx = {row['date_parsed'].strftime('%Y-%m-%d'): idx for idx, row in nifty_df.iterrows()}
            ...
            print(f"Loaded Nifty 50 index ({len(nifty_lookup)} trading days).")
        except Exception as e:
            print(f"Error loading Nifty 50: {e}")
            return
    else:
        print(f"Error: Nifty calendar not found at {nifty_path}")
        return
    ...
    files = [f for f in files if "NIFTY_50_day" not in os.path.basename(f)]
    print(f"Processing {len(files)} stock files in Nifty 500 database...")
    
    for count, f in enumerate(files):
        symbol = os.path.basename(f).replace("_day.csv", "").upper()
        if (count + 1) % 50 == 0 or (count + 1) == len(files):
            print(f"Progress: {count + 1}/{len(files)} files processed.")
        ...
        try:
            ...
            df['date_parsed'] = pd.to_datetime(df['date'].astype(str).str.slice(0, 10))
            ...
            for idx, row in sig_df.iterrows():
                sig_date_str = row['date_parsed'].strftime('%Y-%m-%d')
                sig_year = row['date_parsed'].year
    ...
    print("Class Distribution (Target > +25%):")
    ...
    output_path = os.path.join(results_dir, "nifty500_10y_training_data.csv")
    ml_df.to_csv(output_path, index=False)
    print(f"Successfully saved Nifty 500 ML training dataset to: {output_path}")
